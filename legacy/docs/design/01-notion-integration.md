...
#### 4.2.1 批准的措辞（2026-09-18 增补）

「批准」这件事一直由四条整串相等的字符串判定（`批准` / `确认` / `approve` / `approved`），Epic 侧同形。这条判据的失败方式不是报错而是**改写**：写「批准。」带一个句号，PRD 落进 `request_revision`，系统当作"你要求改"重写一版，而人从未被告知他写的那个词没算数；Epic 侧更糟——不匹配的评论落到 `feedback`，也就是沉默，Epic 继续等，没有任何一处说明它在等什么。

所以加一条 Noul（`src/judge/approval-intent.ts`），**方向与 §8.6 的环境判断相反、理由同构**：那边判官把拒绝理由从代码侧移走，错了只多跑一轮；这边判官把一条评论变成批准，错了就是**没人批准过的东西被拿去建，而人真正提的要求丢了**。所以白名单是地板、判官只许往"是批准"这一个方向加、且门槛高（`judge.approvalThreshold`，默认 0.8）。白名单已经认出的措辞**不送判官**——它今天是批准，明天还是。

阈值由实测定：真批准落在 0.83–0.96（最弱的是「确认，可以往下走」0.87），同意里带要求的落在 0.02–0.17（「整体可以，但第二条再写细一点」0.02），而**光夸不表态**落在 0.55–0.76（「这个写得不错」0.55、「挺好的」0.76）——后者正是这道门槛要挡住的那一类，0.8 把它留在门外。采集的往返存进 `fixtures/judge/approval-comments.json`，问题措辞一改测试即红，因为这个数字是对着那批措辞量出来的。

一条评论一个请求，理由同 03 §8.6：批量问会让一个人的批准取决于同一轮别人写了什么。判官缺席、超时或不确信一律等于没有意见，白名单的结论就是全部结论——也就是今天的行为。每条被移动的评论记 friction（`notion_approval_judged`），用数据回答这道判断值不值得留。

#### 4.2.2 拆解方案上的评论（2026-09-18 增补）

与 §4.2.1 同源的第二个缺陷，但更糟：Epic 的 `PLAN_APPROVAL` 评论只认四条字符串，其余**落到 `feedback`，也就是什么都不做**。人写「这个拆解不对，第二张卡应该拆成两张」，页面毫无反应，Epic 继续等，没有一处说明它在等什么。

改成与需求页同一读法：**方案在页上等批准时，人写的任何不是批准的东西，都是要求改这个方案**，并且把话带走。

第二个缺陷藏在第一个后面：`requestRevision` 原本只把 Epic 退回 `DECOMPOSE`，**不记录人说了什么**。于是连走得通的那条路（「请修改拆解方案」）也是一次盲目重试——同一段需求必然产出同一个方案，人看着自己否掉的东西原样回来。现在退回意见落成 `epic.plan_revision_requested` 事件，与状态迁移同一 batch，下一次拆解读回来接在 requirement 后面（与阻塞回答同一形态）。

一轮轮询里关于同一个方案的评论**一起**进一次退回，并**全部** claim 掉：留一条没 claim 的，下一版方案一到就会被它立刻退回。批准若写在某条要求修改之后则不算批准——页上现在这一版不是他批准的那一版。
