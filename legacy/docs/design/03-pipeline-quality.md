...
## 13. 增补（2026-09-20）：屏幕的存在与否是代码问题，不是浏览器问题

### 13.1 病灶：写好了没接上，每一层都太晚或看错了服务器

S-R237511MB-02 九个屏幕场景全部判"通过"，而这张卡存在的理由里有三个页面（`/operator/costs`
`/operator/records` `/operator/roles`）从产品入口根本到不了。组件写好了、单测全绿——测试测的是那个注册
函数，不是"入口起来之后这个路径能访问"。

三处叠加让它漏过去：

1. CODE 出口（§8.1）只问树干不干净、红绿有没有跑、场景有没有被标记，不问"这一页进不进得去"。
2. 场景 `layers` 是 `["e2e","ui"]`，于是**"这一页在不在"这件 code 层完全证得了的事，被整条场景一起交给
   了浏览器道**。`applyDowngrades`（§12.2）只朝一个方向开口：code 证不了 → 交给浏览器，反方向没有出口。
3. 那一轮还没有应用道，提示词让验证者自己起服务。自起的服务器是按代码里找得到的注册函数拼出来的，
   **所以产品入口没挂的路由在它上面是存在的**。
...
| 层 | 改动 | 位置 |
|---|---|---|
| verdict 代码校验 | 屏幕场景报的页面地址必须与应用道同源；loopback 拼法折叠，端口承载含义 | `validateVerdict` 的 `appUrl` |
| DoD | 屏幕场景必填 `page`（应用里的路径），与 `visible[]` 同一轮要 | `scenariosMissingPage`，SHAPE 出口 |
| CODE / REGRESSION_FIX 出口 | 起仓库声明的应用，逐个打开 `page`，findings 回喂同一 session | `screen-reachable` gate |

第一条补的是 §9 的三层防造假里缺掉的那两层：此前"不许把自己起的服务当成被验证的应用"只有提示词一层，
而 host 白名单是主机名级的，应用道的端口和验证者随手起的端口都在 localhost 上。

### 13.3 判据为什么这么窄

CODE 出口只有 **404/410** 和**完全没应答**会拒。302 到登录页、401、500 都说明路由挂上了；要求 200 会让
每个需要登录态的页面依赖这一阶段造不出来的数据，那是造数命令要解决的问题，不是一张卡能解决的。

仓库没声明启动命令、或这一轮应用起不来，都什么都不问：此刻"箱子起不来"和"代码把启动搞坏了"是同一个
观测，为它打回会让一个被占住的端口卡住所有卡。VERIFY 那一轮仍会报 inconclusive（§9.3），不会静默通过。

`page` 在 schema 里是 optional：此前冻结的 DoD 仍然要能解析，它们的屏幕场景没有 `page`，这道关对它们
不问任何问题。新卡由 SHAPE 出口强制。

### 13.4 记数

每次打回记 `screen_not_mounted` friction。一条从不触发的规则说明提示词本来就够了，一条每张卡都触发的
规则说明提示词不是该修它的那一层。
