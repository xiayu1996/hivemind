...
| M0-11 | C1 登录+刷新 | **PASS（活体）** | 已登录 ready；强制刷新轮换成功；真实 Codex 跑通一轮 |
| M0-12 | C2 usage-limit 解析 | **PASS（源码级）** | 解析器 + 8 单测全绿，模板由二进制反推非猜测 |
| M0-13 | C3 探针零副作用 | **PASS（活体）** | 5x `--no-refresh` 后 mtime/size 未变 |
| M0-14 | C4 并发刷新锁 | **PASS（活体）** | 3 并发强制刷新无 invalid_grant，事后仍 ready |
