...
- **一张卡加的东西，做完时要从产品里到得了**：Story 是竖切，所以本轮新增的导出如果除了它自己那个文件和测试之外全仓无人提及，它就只活在自己的单测里——而单测会导入它要测的东西，所以单测全绿证明不了这件事。S-R237511DT-04 写了 `registerOverviewRoutes` 而入口从未调用，同一路径上服务的仍是原来那一页，三条场景全在对着别人的页面判定，烧完 3 轮预算（2026-09-22）。CODE 与 REGRESSION_FIX 出口因此先问这一条（不用起应用、与渲染方式无关，比屏幕探针便宜），再问可达性；只拒**一处都没有**的那些——有一处提及就是接线决定，这道门没有资格评判。

- **应用道是见证，被这一轮改写过的见证不算见证**：屏幕可达性靠"起仓库声明的应用"来证明，而这只在被起的那个东西是产品时成立。R237511RC 把角色配置页的数据端口**只**加在了 `verify.appStartCommand` 指的那个启动脚本里，产品中持有中央库、人真正打开的那个挂载从来没接上——于是每一页都可达、每一轮都绿，而这页在产品里根本不存在（2026-09-22）。所以有屏幕场景的一轮不许改动启动命令指向的入口文件：挂一个新页面属于入口加载的路由装配，不属于"怎么把产品起起来"。检查在探针之前跑（witness 被改写了就不必再花一次探测），findings 回喂同一 session；判据只看路径相交，命令里没有仓库内文件路径（`npm run dev`）就什么都不问。
