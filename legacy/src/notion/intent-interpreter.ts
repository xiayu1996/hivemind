 ...
 *
 * `approving` holds comment bodies a judge read as approvals, for the wordings
 * the four-string whitelist below cannot match ("批准。" with a full stop,
 * "确认，可以往下走"). It only ever widens: a comment the whitelist already
 * recognises is never asked about, and a caller with no judge passes nothing
 * and gets exactly today's reading.
 */
export function interpretRequirementComment(
  state: RequirementState,
  body: string,
  approving: ReadonlySet<string> = new Set(),
): RequirementCommentIntent {
  const text = body.trim();
  if (text === "") throw new Error("a Notion comment cannot be interpreted without text");
  if (state === "CLARIFY") return { type: "answer", body: text };
  const yes = approved(text) || approving.has(text);
  if (state === "PRD_CONFIRM") {
    return yes ? { type: "approve_prd" } : { type: "request_revision", body: text };
  }
  ...
  if (state === "SOLUTION") {
    return yes ? { type: "approve_solution" } : { type: "request_solution_revision", body: text };
  }
