...
describe("captured provider failures", () => {
  ...
  it.each(providers)("%s: covers the classes the gate requires", (provider) => {
...
describe("the coverage gate", () => {
  ...
  it("passes a metered provider whose one provokable failure was captured", () => {
    expect(() => assertErrorFixtureCoverage(["openai-codex", "deepseek"])).not.toThrow();
  });

  it("refuses a provider added to the chain before any failure was captured", () => {
    // Adding a provider is a configuration change; without this it is enough to
    // get a card assigned to one whose error surface nothing has ever read.
    expect(() => assertErrorFixtureCoverage(["openai-codex", "anthropic"]))
      .toThrow(/anthropic: AUTH/);
  });
});
