// One-shot operator fix: collapse the double S- prefix the seed rename left
// on the task ids, keeping the DoD story-id grammar intact.
...
do {
  ...
  for (const page of data.results) {
    ...
    if (!id.startsWith("S-S-")) continue;
    const corrected = id.replace(/^S-S-/, "S-");
    await gateway.request({
      method: "PATCH",
      path: `/v1/pages/${encodeURIComponent(String(page.id))}`,
      priority: "interaction",
      body: { properties: { "任务 ID": { rich_text: [{ type: "text", text: { content: corrected } }] } } },
    });
    fixed++;
