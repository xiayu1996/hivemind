...
/** Suffix of a log that has been packed; the plain file is gone when it exists. */
export const CANONICAL_LOG_SUFFIX = ".zst";

/**
 * A window wide enough to reach the previous copy of the conversation.
 *
 * Every provider request in one of these logs carries the whole conversation
 * that came before it, so the file is mostly one text repeated at growing
 * lengths. The redundancy spans megabytes, far past a deflate window: gzip
 * gets under 4x on a real log where this gets past 200x, and the difference is
 * a 139MB file becoming 690KB rather than 36MB.
 */
const WINDOW_LOG = 27;

/** Refuses a log that would not fit in memory rather than truncating it. */
const MAX_UNPACKED_BYTES = 1_024 * 1_024 * 1_024;

/**
 * Packs a finished log and removes the plain file.
 *
 * Only ever called once a log is complete and has been proved to round-trip:
 * the writer appends line by line and a packed file cannot be appended to, so
 * packing a log still being written would lose everything after it.
 */
export async function packCanonicalLog(path: string): Promise<void> {
  ...
  await writeFile(`${path}${CANONICAL_LOG_SUFFIX}`, packed);
...
export async function readCanonicalLog(path: string): Promise<CanonicalEvent[]> {
  return parseCanonicalLog(await readCanonicalLogText(path));
}

async function readCanonicalLogText(path: string): Promise<string> {
  try {
    ...
    // mentioned.
    const packed = await readFile(`${path}${CANONICAL_LOG_SUFFIX}`).catch(() => {
      throw cause;
    });
