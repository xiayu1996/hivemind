...
export const CONFIG_KEYS = {
  ...
  "judge.enabled": def({
    ...
    scope: "global",
    reload: "hot",
    description: "Whether the structured judge (a second model path, outside pi) may be asked the questions that declare a fallback. Off means every such question keeps the deterministic answer it has today, so a deployment without the credential behaves exactly as before. On requires TYPESAFE_API_KEY in the secrets file.",
  }),
  "judge.endpoint": def({
    schema: z.string().url(),
    ...
    scope: "global",
    reload: "hot",
    description: "Where the structured judge is reached. A URL rather than a constant for the same reason provider profiles are data: pointing it at a recorded stand-in is how the judged paths are exercised without a credential.",
  }),
  "judge.model": def({
    schema: z.string().trim().min(1),
    default: "jev-latest",
    scope: "global",
    reload: "hot",
    description: "Which judge model answers. Unlike the pi chain there is no failover: a judge that is unavailable costs its caller an opinion, never a round, so a second provider would buy nothing.",
  }),
  "judge.timeoutMs": def({
    schema: positiveInt.max(30_000),
    default: 8000,
    scope: "global",
    reload: "hot",
