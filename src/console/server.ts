...
import {
  matchedConsoleWriteRoute,
  TODO_DECISION_PATH,
  TODO_SAVE_CHECK_PATH,
...
export async function createConsoleServer(
  ...
  if (options.todoCommands) {
    writable.add(TODO_DECISION_PATH);
    writable.add(TODO_SAVE_CHECK_PATH);
  }
  app.addHook("onRequest", async (request, reply) => {
    if (request.method === "GET" || request.method === "HEAD") return;
    const route = matchedConsoleWriteRoute(request.method, request.url);
    if (route !== null && writable.has(route)) return;
