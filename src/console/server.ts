...
export async function createConsoleServer(
  ...
  // The overview page reads the whole store and states the same alert, so it
  // owns the root whenever the mount point handed one over; this stays for the
  // hosts that mount the ledger alone.
  if (options.overviewPage === undefined) {
  ...
  // The overview is server-rendered from the store, so like the costs page it
  // is served by the process holding it rather than behind `serveUi`: a host
  // with the store and no built shell still owes a person the first screen.
  ...
  if (options.serveUi !== false) {
    ...
    if (uiRoot !== null && index !== null) {
      ...
      // A built shell owns the remaining pages; `/` and `/costs` are rendered
      // above whether or not a build ran, and `/overview` belongs to the
      // overview page whenever the mount point handed one over.
      for (const route of [
        ...(overviewPage ? [] : ["/overview"]),
        "/nodes", "/tasks", "/config", "/stats", "/providers", "/queue",
      ]) {
