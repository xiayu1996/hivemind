 ...
 *
 * `getComputedStyle` is read once per element and only for the properties the
 * checker judges: asking for the whole declaration returns several hundred
 * entries per element, almost all of them browser defaults.
 */
export const COLLECT_STYLES = `(input) => {
  ...
  return {
    collect: async (url, properties, maxElements) => {
      ...
      try {
        ...
        return await page.evaluate(COLLECT_STYLES, { properties, maxElements }) as PageStyles;
      } finally {
        await page.close();
      }
    },
