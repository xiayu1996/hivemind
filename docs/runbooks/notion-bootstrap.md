...
| 视图 | 类型 | 配置 |
|---|---|---|
| 看板 | board | 按 `AI 状态` 分组，六列顺序 `待启动 → 进行中 → 需要输入 → 人工停靠 → 已完成 → 失败`；卡片显示 `等待人`、`执行阶段`、`优先级`、`Epic`、`目标仓库`、`轮次`、`成本(USD)`、`MR`；排序 `优先级` 升序、`最近更新` 倒序 |
| 🙋 需要我 | table | 过滤 `AI 状态 ∈ {需要输入, 失败}`，按 `最近更新` 升序（等得最久的在最上） |
...
3. **🙋 等我处理**：三个 linked **list** 视图（table 会撑出横向滚动条），分别过滤 需求 `∈ {澄清中, PRD 待确认, 待验收}`、Epic `= 拆解待确认`、Story `∈ {需要输入, 失败}`，都按 `最近更新` 升序，卡片只显示 `等待人` 与状态。三个都为空即 agent 没有在等人。
