...
| MU-02b | 谁来写这三件套（2026-09-18 定为候选①）：需求层加 `PROTOTYPE` 档，紧跟 SOLUTION、仅 `interface` 非空时跑；guard 把写权限围在 `prototype.root` 之内，产出以 PR 进目标仓库，人确认方案时一并确认。`prompts/pm/prototype.md` 方法层（已起草）：先判界面类型（操作/说服/阅读/体验，操作型默认克制）、色彩策略先于颜色、两遍走并对着五种默认样自批、界面文案规则、有界自检一轮、设计理由永不进 HTML 注释与隐藏节点。否掉"人手工放初稿"：MU-07 起的每条出口检查都要有一份能回喂重画的系统产出 | `src/orchestrator/prototype-runner.ts`（新，复用 `requirement-draft.ts` 的起草循环）、`requirement-machine.ts`、`src/guard/policy.ts`（`PROTOTYPE` 档与路径围栏）、`prompts/pm/prototype.md`、`src/vcs/`（原型 PR）、08 §3.1 | `prototype-runner.test.ts`（`interface=null` 不跑、围栏外写入被拒并记 friction、产出落 PR 而不直推）；`policy.test.ts`（`PROTOTYPE` 只能写 `prototype.root`）；一条真实需求跑出一份能被 MU-02 读法整份注入的契约 | MU-02, MC-03 |
| MU-03 | Notion 方案区段：改了什么 / 方案摘要 / 页面清单 / 逐页四态截图 toggle / 待定分叉 to_do / 可点原型 embed / 确认清单（两类意图与看板新列已随 MU-01 落地，这一片补页面呈现与勾选回读） | `src/notion/blocks/requirement-page.ts`、`requirement-projection.ts`、`requirement-input-sync.ts`、`intent-interpreter.ts`、`display-text.json`、`board-status.ts` | `notion-write-language.test.ts`（裸枚举/英文模板即红）；`requirement-input-sync.test.ts`（勾选回读改状态）；`intent-interpreter.test.ts`（两类新意图）；原位迁移用例（改版不重建 blockId） | MU-01 |
...
| MU-07 | 原型出口·确定性部分（08 §3.2 前三行）：页面清单里每页声称承接的 PRD 场景，其角色与文本必须出现在原型页的 aria 快照里（跑 MU-05 的断言）；每页必须能以 `?state= 取 empty / loading / error / waiting` 切四态，截图道逐态打开、渲染不出即缺；计算样式只许来自 `tokens.json`（复用 MU-06）。三条都能否决，用尽 `solution.maxRounds` 即 fail、需求留在 SOLUTION 等人 | `src/orchestrator/prototype-runner.ts`（gate 表）、`src/verify/snapshot-assert.ts`、`src/verify/prototype-screenshots.ts`（按 `?state=` 采集，不再由模型构造状态）、`src/pipeline/interface-contract.ts` | `prototype-runner.test.ts`（缺一个场景角色即回喂、四态缺一即回喂、写死色值即回喂、三轮用尽 fail 不新增停点）；`prototype-screenshots.test.ts`（四态 URL 逐一打开） | MU-02b, MU-05, MU-06 |
| MU-08 | axe-core 进契约层：原型出口与 VERIFY 共用一份跑法，`serious` / `critical` 违规记违例；随 `uiContract.enforce` 三态走（原型出口侧恒为 block，因为原型没有"先看一条需求再说"的问题） | `src/verify/accessibility-audit.ts`、`token-conformance.ts`（合并为契约层一次评估）、`config/registry.ts`、08 §6 | `accessibility-audit.test.ts`（以一页故意缺 label 的 fixture 为红、修好为绿；`moderate` 以下不算违例） | MU-07 |
| MU-09 | 审美一次仓库级决定（08 §3.3）：`interface.direction {summary, alternatives[]}` 进 SOLUTION 产出与停人逻辑；目标仓库尚无契约时原型档对同一页画 `prototype.directionVariants`（默认 3）个方向并截图，Notion 方案区段并排供人只勾一个，选中结果落 `requirement_acceptance_items`；`design.md` 理由层（DESIGN.md 九节骨架：视觉主题与界面类型/色板与角色/字体规则/组件样式/布局原则/层级与深度/该做与不该做/响应式/给后续卡的提示）整篇注入，引用不存在的 token 名即整份契约不注入；原型档 prompt 加反均值禁用清单与"大胆花在一处"原则，机械可判的部分接 impeccable 独立检测引擎（`impeccable detect --json --no-config` 文件模式，Apache 2.0；pin engine 版本线 `hivemind.impeccableEngineVersion`，`install.sh` 从 release 取 linux 二进制并校 sha256、不走 npm 启动器与首次运行下载，`preflight` 探可执行且版本等于 pin）作 warn 级：读 `--json`、忽略退出码 2、finding 全记 friction，退出码 1 只报告；§3.1 自批结论同样只记 friction | `prompts/pm/{solution,prototype}.md`、`src/verify/design-lint.ts`（检测器封装）、`deploy/linux/install.sh`、`scripts/preflight.ts`、`package.json`、`requirement-solution.ts`（`direction` schema）、`prototype-runner.ts`、`src/pipeline/interface-contract.ts`（`design.md` 解析与 token 名校验）、`src/notion/blocks/requirement-page.ts`、`requirement-input-sync.ts`、`display-text.json` | `requirement-solution.test.ts`（`interface` 非空而无 `direction` 即拒、无备选即拒）；`interface-contract.test.ts`（`design.md` 引未知 token 即缺项、重排后注入逐字节不变）；`prototype-runner.test.ts`（无契约仓库出 N 个方向、已有契约跳过）；`design-lint.test.ts`（退出码 2 记 friction 不否决、退出码 1 只报告、版本不等于 pin 即探针红、字面版本号不出现在仓库任何 shell 或 ts 里）；`requirement-input-sync.test.ts`（勾第二个方向时前一个自动取消） | MU-03, MU-07 |
| MU-10 | 可用性清单 gate，先分流（08 §3.2）：机械条目（`prefers-reduced-motion`、触控目标尺寸、焦点可见、键盘可操作）归代码与 axe-core，能否决、用尽即 fail；语义条目（状态文案是否说清发生了什么与该做什么、校验提示是否可操作、标签是否描述旁边那个控件）由判官模型逐条二值判定，输入是剥掉注释与 `<script>` 的原型 HTML + MU-07 采的 aria 快照，**不是截图**；finding 以条目编号为键、不引编号即丢弃；同一条目连续两轮翻转记 friction `ui_checklist_unstable`；能否决，用尽 `solution.maxRounds` 即 `ship`。判官走 `src/judge/`：每条语义条目地板为"未见问题"，判官只能加 finding、不能动机械条目的结论，不可用即地板（AGENTS.md 判官不变量）；阈值单独一键 `judge.usabilityThreshold`、保守方向是高（加 finding 即打回），不与 `judge.environmentThreshold` 共用；上线前在真实中文原型样本上量 | `prompts/pm/ui-checklist.md`、`prototype-runner.ts`、`src/pipeline/ui-checklist.ts`（条目解析、编号校验、HTML 剥注释）、`src/verify/usability-mechanical.ts`、`src/judge/usability.ts`（三条语义问题的 Noul 定义与地板） | `ui-checklist.test.ts`（无编号的 finding 被丢弃、引清单外编号被丢弃、注释里的自夸文本不进判官输入、条目翻转记 friction、判官不可用即 ship）；`usability-mechanical.test.ts`（每条机械条目红绿各一例） | MU-07, MU-08 |
...
| MJ-02 | 真实往返验收：本机 `~/.hivemind/secrets.env` 补 `TYPESAFE_API_KEY`，跑一次真实请求确认契约与文档一致；在真实中文拒绝理由上量 `judge.environmentThreshold`（判官文档写明 English is primary，其他语言准确率更低，不能照搬示例阈值）。采一份真实响应进 `fixtures/`。**在此之前 `judge.enabled` 不得置 true** | `fixtures/judge/`、`scripts/preflight.ts`（判官凭据探针）、`config` 阈值定值 | 一次真实往返；一批真实中文理由上的逐条人工核对（至少覆盖历史上被漏判的两种措辞） | MJ-01 |
| MJ-03 | Notion 批准意图：`intent-interpreter.ts` 的 `approved()` 现在是四条字符串整串相等（「批准。」带句号即落进 `request_revision`，PRD 被当成"你要求改"重写一版），`interpretEpicComment` 同形。改为按状态选问题的 Choice，地板是现有白名单，低置信度往"要求修改"落而**永不**自动批准。判断在 ingest 当场做一次并落库，prompt 重建读那一行，不重判 | `src/judge/approval-intent.ts`（新）、`notion/intent-interpreter.ts`、`comment-ingest.ts`、`config/registry.ts`（`judge.approvalThreshold`） | `intent-interpreter.test.ts`（白名单原样通过、带标点与口语化的确认被读成批准、含要求的句子永不读成批准、判官缺席即退回白名单） | MJ-02 |
| MJ-04 | 拆解语言判据：`decompose.ts` 的 `implementationLanguage` 是 11 个英文词 + 6 个中文词的表，过每张 Story 的 title / requirement / given-when-then / userEntryPoint / verificationPath。假阴性（「引入缓存层以降低响应延迟」零命中）与假阳性（需求本身就是关于开放接口的产品）都发生过，而 `EpicDecomposer` 只有 2 次 attempt，同一条误判拒两次即 BLOCKED。加一条 Noul「这句描述的是用户能做什么，还是系统内部怎么实现」，地板是现有表 | `src/judge/business-language.ts`（新）、`orchestrator/decompose.ts` | `decompose.test.ts`（表命中的照旧拒、全中文的实现句被判官拒、业务上正当的技术名词不再被误拒、判官缺席即表的结论） | MJ-02 |
| MJ-05 | 垂直切片判据：`userEntryPoint` / `verificationPath` 现在只判非空，挡不住敷衍的一句话（2026-09-05 那个 Epic 六张卡各建一层、没有一张能独立验证）。加一条 Noul「这个 Story 做完之后，一个人能不能在不等其他 Story 的情况下打开某个界面看到它的结果」，地板是现有非空检查 | `src/judge/vertical-slice.ts`（新）、`orchestrator/decompose.ts` | `decompose.test.ts`（以 2026-09-05 那次真实拆解为 fixture，逐张卡必须被判为非垂直切片） | MJ-04 |
| MJ-06 | 给人读的那一句的语义层：`lintHumanSentence` 判的是 CJK 占比与六条正则，拦得住堆栈帧、拦不住全中文的实现句。加一条 Noul 追加 findings，**只挂在 `exhausted: "ship"` 的 gate 上**（设计摘要、交付报告），所以概率判断永远停不住一张卡 | `src/judge/human-sentence.ts`（新）、`report/business-language.ts`、`orchestrator/pi-phase-port.ts` | `pi-phase-port.test.ts`（判官加的 finding 会触发重写但用尽轮次仍照发）；`business-language.test.ts`（判官缺席即现有结论） | MJ-02 |
