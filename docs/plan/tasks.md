...
| MU-02b | 谁来写这三件套：SOLUTION 是只读档（`READ_ONLY_PHASES`），没有任何一层能把原型写进仓库。两条候选——① 需求层加一个只能写 `prototype.root` 的原型档（受 guard 路径围栏），产出后开 PR 再交人确认；② 由人手工放一版初稿、系统只做读与校验。选定前，涉及界面的需求会停在「方案待确认」而没有可看的原型 | 待定 | 待定 | MU-02 |
