...
MU-09 拆开的那一项（2026-09-18）：“仓库首次建立契约时对同一页画 `prototype.directionVariants` 个方向、人在 Notion 方案区段里只勾一个”留到 MU-03：
它的产出只有一个消费者——方案区段的并排截图与勾选回读，而那一片本身还没做；先把四个方向画进仓库而没人能选，就是写一段没人读的产物。
