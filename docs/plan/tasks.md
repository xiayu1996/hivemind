...
| W-54 ✅ | W-45 把「应用怎么起」从会话的猜测换成了仓库的事实，但本仓库**没有一个可以被启动的应用入口**：console 只有 `createConsoleServer`，唯一起它的地方是 orchestrator 常驻自己。于是 `verify.appStartCommand` 只能留空，所有 `ui`/`e2e` 场景拿到的是一个稳定的「这里没有应用」——比此前的随机结论好，但还是验不了 | 缺的是一个长期入口，不是一处逻辑 | 新增 `scripts/serve-console.ts`：只起控制台、**构造上只读**（不传 configWriter，写面就不注册，server 自己的钩子对非 GET 回 405），直连中央库而不是拷贝——被判的是真实轮次、真实费用、真实卡点的那些屏，一个刚迁移出来的空库会把每一页都渲染成空状态，而「空状态」恰恰是这些场景要分辨的东西之一。库地址缺失时**拒绝启动**（libsql 会凭空建库，那样看起来像个能用的应用）。顺带把 orchestrator 传给子进程的 `HIVEMIND_DB_URL` 绝对化：Story 子进程的 cwd 是仓库根，而验证轮起的应用 cwd 是**被验证的那棵 worktree**，那里根本没有 `data/`。配置随之写上（`xiayu1996/hivemind` 的 `verify.appStartCommand` / `appReadyUrl`），实测起停一遍：真实页面 200、`stop()` 后端口不再应答 |
| W-55 ✅ | Epic R237511TD 的五张回归卡卡死：开卡时间 13:10，五张**同一个签名**（W-46 那个共享占位符留下的），归因二分的结论是 `not_reproduced`——它在任何一个被探的 revision 上都没复现出来，于是 `attributed_story` 永远是 null。而 `resolveCard` 要求「关卡的人必须是这张卡归因到的那张 Story」，`openCardsForStory` 也只认归因过的卡：**系统里没有任何一个角色关得掉它**。闸门那一侧却是按 `scenario_registry` join 的，不看归因，所以照样挡住 Epic 的评审请求——哪怕这五条场景在 13:22 的那次清扫里全部**通过**了 | 开卡的规则写在 `judgeRegression` 里：「卡描述的是**正在发生**的断裂；最新一次是绿的就没有东西可复现」。这条规则只用在开卡，从没反过来用在关卡，于是卡活得比它成立的那个条件更久 | 把同一条规则反过来用一次：观察窗口内一次失败都没有（`stable`）时，该场景的未关卡片由清扫自己关掉，不问归属——是清扫开的，就由清扫关。这不会放红的头过去，因为闸门的第二条腿（**评审请求要提的那个 revision 上必须有一次通过**）才是真正守住 Epic 的那条，而它一动不动。抖动的场景窗口里仍有失败，卡不会一绿就关。第二半：把清扫的 triggered 集合从 `unprovenScenarios` 换成 `scenariosAwaitingDelivery`（未通过的 ∪ 带卡的）——关卡要的证据只有清扫产得出来，而 7x24 的机器不会空闲，不放进 triggered 就等于等一个不会来的空闲 |
| W-56 ✅ | W-54 说 `serve-console.ts`「构造上只读：不传 configWriter，写面就不注册」——**这句话是错的**。`createConsoleServer` 里有一句 `options.costLimitStore ?? data.requirementCostLimitStore`，而 `LibsqlConsoleDataSource` 永远自带那个 store。于是这个为验证轮起的、一个端口都没传的进程，对着**中央活库**开着一个能改需求费用上限的表单，导航白名单又正好放行 127.0.0.1 | 回落这件事本身：写面因为「读的人碰巧也写得动」而出现，而不是因为有人要它。todo-contract 自己写着「写面是一张清单不是一条规则，验收就是**读一行**能确认再没有别的写」——回落恰恰让这件事读一行确认不了 | 去掉回落：只认挂载点交过来的端口，orchestrator 那个唯一该写的挂载点显式传 `consoleData.requirementCostLimitStore`。顺带改掉 `S-R237511CO-03-setlimit` 那条场景：它的名字叫「**没有显式写入端口时**费用分析页仍能保存当前需求上限」——「写入端口」是实现词汇，用户没有端口，这条场景把一个实现便利冻成了验收判据。改成「在费用分析页上保存某个需求的费用上限」，用户那一侧的行为一点没变，另补一条「没交过写面的挂载拒绝保存」。实测：`GET /` 200，`POST /costs/requirement-limit` 405 |
| W-57 ✅ | 一张回归卡被归因到某张 Story 之后，那张 Story 会被重开进 `REGRESSION_FIX`，而它在 prompt 里读到的是：「the scenario fails on the Epic branch since S-…: `4e66ec9672d33a6d06b7b5a9e334a2b4`」——**三十二个十六进制字符**，然后被要求去复现它 | `regression_runs` 只存签名不存文本，`normalizeFailureText` 的注释写着它存在的理由是「让读的人看见被归到一起的是什么，而不是一个只能凭信的哈希」，却没有任何地方存它的输出；`RegressionCardRef.signature` 的注释甚至写着「the normalised failure text the card is keyed by」——意图一直是文本，实现给的是哈希 | 两张表各加一列 `failure_text`（截断 2000 字），开卡时从那一轮的失败文本取；prompt 渲染文本而不是键，没有文本时明说「没有记录失败文本（break <键>）」而不是把键冒充成断裂。活库已用 `ALTER TABLE` 补上两列（写入即刻可用），但 ALTER 会把列追加进 sqlite_master 里的建表语句，指纹门禁读的正是那段文本——所以**重启之前**要按迁移原文重建这两张表，脚本与命令已交给 Ryan，实测 178/5 行零丢失、零漂移 |
| W-58 ✅ | `run-story.ts` 按 `story.repo` 取配置，`run-regression.ts` **不取任何作用域**。`verify.appStartCommand` / `appReadyUrl` / `appReadyTimeoutMs` / `guard.e2eHostAllowlist` 全是 per-repo 键——无作用域的 store 在 `reload()` 里**根本不会 SELECT 那些行**，于是清扫一律读到代码默认值（空命令）。结果是：同一条场景，Story 的验证轮有应用可开，回归清扫没有。这正是 W-45 要消灭的「两条道各判各的」，只是它在清扫那条道上活了下来 | 清扫从来不知道自己在扫哪个仓库 | 仓库由数据库**推**出来（`sweepRepository`：有 epic 取 `epics.repo`，否则由场景回溯到各自 Story 的 `repo`，跨两个仓库就报错而不是挑一个——一次清扫只有一棵树一个 revision）。不用「新增必填参数」是因为常驻活得比它的子进程久：加一个必填 flag 是一个部署步骤，而推导在子进程被换掉的那一刻就对了——今天实测过，改成必填当场把一次真实清扫打成了 P0。`--repository` 保留为可选覆盖。另在 `appLaneConfig` 上加一道窄守卫：拿不到仓库作用域就直接报错，而不是把「没人配过的值」当成配置 |
| W-59 ✅ | 清扫的会话记录里躺着一句 W-45 本该消灭的话：「The application could not be started for this round: `no database at data/hivemind.db`」——应用起不来，所以三条界面场景又一次判了个「这一轮不作数」 | `file:data/hivemind.db` 是**相对**地址，而它一路往下传：orchestrator → 清扫子进程（cwd 是仓库根，成立）→ 验证轮启动的应用（cwd 是**被验证的那棵 worktree**，那里没有 `data/`）。W-54 在 orchestrator 里补过一次绝对化，但两个脚本自己的兜底默认值仍是相对的，手工跑、或常驻还没重启时照样踩 | 绝对化收敛成 `src/persistence/client.ts` 的 `absoluteDbUrl`，三个脚本共用——地址在被交出去之前就已经指向一个地方，而不是指向「谁的当前目录」。libsql URL 可能带的 query 保留，`libsql://` 与 `:memory:` 原样不动 |
| W-60 ✅ | 一次 Notion 网络抖动，日志里同时出现两种反应：`intake sync was skipped this cycle after a transient network fault`（对），和 `P0: epic decomposition failed: The operation was aborted due to timeout`（错）——**同一个故障，一个当例行、一个叫人** | `step` 早就会跳过「重试能好的故障」，但拆解和 Epic 维护在它外面又各包了一层 `catch → reportP0`，先接住了就永远轮不到 `step` 判。`step` 自己的注释里写着这件事曾经「为一个下一轮就会成功的请求叫了一次人」，修的是 `step`，而内层的 catch 把它原样又造了一遍 | 三处收敛成一个 `reportedStep`：问分类器「这事要不要人」（`needsHuman`，认不出的一律算要），要人才发 P0，其余只留一行日志、下一轮再来。回归清扫本来就是这么写的，现在拆解与维护跟上。实测四条真实文案：Notion 超时不叫人、订阅打满叫人、「没有符合契约的候选」走 UNKNOWN 叫人 |
| W-61 ✅ | W-59 之后应用终于起来了，失败文本随即变成下一句真话：「这轮跑起来的应用里**没有待办处理页面**（一打开就提示找不到页面）」——控制台的界面是一个构建产物，而 `console-ui/dist` 被 gitignore，worktree 里要么没有、要么是**上一轮**留下的 | `serve-console.ts` 只 serve 不 build：没构建过就没有界面可判，构建过但过期则更糟——验证轮会对着上一轮的界面下结论，而它以为自己在判这一棵树 | 起控制台之前先 `vite build`（0.6 秒），失败就明说「界面构建不出来，没有东西可 serve」而不是端出一个空壳；仓库里没有 `vite.config.ts` 就跳过，服务端直出的那些页照常。实测：main 上（无 vite 配置）跳过构建、`/` 200、写面 405；Epic 树上构建后 `/todos` 由 404 变 **200** |
